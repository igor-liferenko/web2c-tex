...
/* Do not output a space after some characters. */
#define IS_SELF_DELIMITER(ch) ((ch) == ';' || (ch) == '(' || (ch) == '[')
  ...
  char *s;
{
  ...
  /* Omitting the space for self-delimiting characters, like semicolons
     and brackets and parentheses, makes some of the future programs
     that operate on the C output lose.  Sigh.  What a kludge.  */
  fprintf (std, "%s ", s);
  line_pos += len + 1;
    ...
    char *s;
{
    ...
    if (ii == -1)
	fprintf(stderr, "not in symbol table");
    else
	...
	case type_id_tok:
	    ...
	    break;
	case proc_id_tok:
	    fprintf(stderr, "parameterless procedure");
	    break;
	case proc_param_tok:
	    fprintf(stderr, "procedure with parameters");
	    break;
	case fun_id_tok:
	    fprintf(stderr, "parameterless function");
	    break;
	case fun_param_tok:
	    fprintf(stderr, "function with parameters");
	    break;
	default:
    ...
    char *argv[];
{
	...
	break;
    case 'c':
	sprintf(coerce_name, "%s.h", &argv[i][2]);
	break;
    default:
